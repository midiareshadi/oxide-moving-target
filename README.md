...
After #396 was fixed, a cuda-oxide developer found the **same** `unreachable`-on-a-runtime-reachable-path mistake at a second lowering site: diverging panic calls (`.unwrap()` on `None`, `panic!`, etc.), filed as [#407](https://github.com/NVlabs/cuda-oxide/issues/407) and fixed in [#408](https://github.com/NVlabs/cuda-oxide/pull/408). Both fixes are now merged into `main`.
